...
colnames(val_species)[1] ="fb_name"
...
write.csv(match_species,'C:/Industrial Ecology/Jaar 3/Masterscriptie/Databases/Fishbase/match_nutr_1.csv', row.names = F)

match_nutr <- read.csv("match_nutr.csv")

sum(is.na(match_nutr$fb_name)) #430
sum(!is.na(unique(match_nutr$fb_name))) #5474
match_nutr_sp <- unique(match_nutr)
match_nutr_species_fb <- na.omit(unique(match_nutr_sp$fb_name))
...
traits.sp <- species(match_nutr_species_fb, # vector of species names
...
sp_5 <- left_join(val_species, sp_4, by = "fb_name")
sp_6 <- sp_5[!is.na(sp_5$Selenium_mu),] #exlude species with NA values
sp_7 <- sp_6[!duplicated(sp_6$fb_name),] 

sp_8 <- sp_7[c("fb_name",'geom', "spec_code",'Selenium_mu', 'Zinc_mu', 'Protein_mu', 'Omega_3_mu', 'Calcium_mu', 'Iron_mu', 'Vitamin_A_mu') ]


st_write(sp_8,'C:/Industrial Ecology/Jaar 3/Masterscriptie/Databases/Fishbase/nutrient_species_mekong_1.gpkg', layer = "nutrient_species_mekong")
